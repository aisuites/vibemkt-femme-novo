...
async function uploadFileToS3(file, type, uploadUrlEndpoint, createRecordEndpoint) {
    try {
        console.log('Iniciando upload:', {file: file.name, type, endpoint: uploadUrlEndpoint});
        ...
        console.log('Response URL:', urlResponse.url);
        ...
        console.log('Presigned URL obtida:', urlData.success);
        console.log('Data recebida:', urlData.data);
        ...
        if (!orgId && urlData.data.s3_key) {
            ...
            if (match) {
                orgId = match[1];
                console.log('Organization ID extraído do s3_key:', orgId);
            }
        }
        ...
        console.log('Headers enviados para S3:', uploadHeaders);
        console.log('URL S3:', urlData.data.upload_url.substring(0, 150) + '...');
        ...
        if (!createResponse.ok) {
            ...
            console.error('Erro ao criar registro (status ' + createResponse.status + '):', errorText.substring(0, 500));
            try {
